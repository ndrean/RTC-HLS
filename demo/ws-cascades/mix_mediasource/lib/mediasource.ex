...
defmodule FrameCapturer do
	def start_ffmpeg(frame_rate, resolution) do
...
defmodule VideoSegmenter do
  require Logger
	...
	def create_video_segment_and_playlist(frame_files, frame_rate) do
    Logger.debug("FFmpeg_rebuild ----#{length(frame_files)}")
    ...
    ffmpeg_rebuild_cmd = ~w(
      ...
      -c:v libx264 -preset veryfast -hls_time 2 -hls_playlist_type event -hls_flags delete_segments+append_list
